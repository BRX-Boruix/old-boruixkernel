...
const COMMAND_ABOUT: &str =
    "testpar\n\nRun testsmp1 in parallel.\nUsage: testpar <workers> [count]";

pub const CMD: Command = Command {
    name: b"testpar",
    usage: "testpar <workers> [count]",
    desc: "Run testsmp1 in parallel",
    about: COMMAND_ABOUT,
    run: run,
};
...
fn run(args: &[&[u8]]) {
    if args.len() < 2 {
        println3("usage: testpar <workers> [count]");
        return;
    }

    let workers = parse_int(args[1]);
    if workers <= 0 {
        println3("invalid workers");
        return;
    }

    let count = if args.len() >= 3 { parse_int(args[2]) } else { 100000 };
    if count <= 0 {
        println3("invalid count");
        return;
    }
