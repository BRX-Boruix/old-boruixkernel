...
const COMMAND_ABOUT: &str =
    "memstress\n\nRun heap stress test.\nUsage: memstress [mb] [chunk_kb]";

pub const CMD: Command = Command {
    name: b"memstress",
    usage: "memstress [mb] [chunk_kb]",
    desc: "Run heap stress test",
    about: COMMAND_ABOUT,
    run,
};
...
fn run(args: &[&[u8]]) {
    let mut buf = [0u8; 32];
    let mut len = 0usize;
    buf[..9].copy_from_slice(b"memstress");
    len += 9;

    if args.len() >= 2 {
        let mb = parse_int(args[1]);
        if mb > 0 {
            let mut packed = mb as usize;
            if args.len() >= 3 {
                let chunk_kb = parse_int(args[2]);
                if chunk_kb > 0 {
                    let mb_u16 = (mb as usize).min(0xFFFF);
                    let ck_u16 = (chunk_kb as usize).min(0xFFFF);
                    packed = (mb_u16 << 16) | ck_u16;
                }
            }
            buf[len] = b':';
            len += 1;
            len += write_num(&mut buf[len..], packed);
        }
    }
    buf[len] = 0;
