use super::Command;
use user_lib::{sys_cpu_count, sys_ps, sys_write};

const COMMAND_ABOUT: &str = "top\n\nShow CPU count and process list.\nUsage: top";

pub const CMD: Command = Command {
    name: b"top",
    usage: "top",
    desc: "Show cpu count and process list",
    about: COMMAND_ABOUT,
    run: run,
};

fn run(_args: &[&[u8]]) {
    let cpu = sys_cpu_count();
    if cpu > 0 {
        print_num("CPU(s): ", cpu as isize);
    }
